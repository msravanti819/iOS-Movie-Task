...
class ViewController: UIViewController {
    ...
    var cellIndex = 0
    var pageNo: Int = 1
    var totalPages: Int = 1  //By default for Top Rated
...
//        view.backgroundColor = .lightGray
       ...
       activityIndicator.centerYAnchor.constraint(equalTo: view.centerYAnchor).isActive = true
        moviemanager.performRequest(userpreffered: "top_rated" , page: pageNo) {
            (Bool,datach) in
            if Bool {
                self.didUpdate(datach: datach)
            }
            print("totalPages = ",self.totalPages)
        }
    }
    
    func didLoad(userOption: String, pageNo: Int) {
        
        moviemanager.performRequest(userpreffered: userOption, page: pageNo) { (Bool, datach) in
            if Bool {
...
extension ViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        activityIndicator.startAnimating()
        cell.MovieName.text = data[indexPath.row].title//data[indexPath.row].title
...
extension ViewController: UICollectionViewDelegate {
    ...
    func updateImagesWithPageNumber(index: Int) {
        ...
        didLoad(userOption: "top_rated", pageNo: pageNo)
    }
}
